...
export default function SettingsStepper() {
  ...
  const [auth, org, resources] = useSelector(state => [state.auth,state.org,state.resources.list])
  ...
  const handleOnChange = useCallback((token) => {
    console.log('Handle on change', steps[activeStep]?.completed,token)
      ...
      >
          ...
          {steps.map((step, index) => {
            ...
            return (
              <Step key={step.label} {...stepProps}>
                <StepButton disabled={!steps[index]?.completed && !steps[index - 1]?.completed} color="inherit" onClick={handleStep(index)}>
                  {step.label}
                </StepButton>
              </Step>
            );
        ...
        {activeStep === steps.length ? null : (
          <>
            ...
            <Paper sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, p: "1rem"}}>
              <Grid
                container
                direction="row"
                justifyContent="center"
